...
All the environments are registered in the `visual_pick_and_place` extension. They start
...
with contextlib.suppress(ImportError):
    import visual_pick_and_place.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~visual_pick_and_place.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from visual_pick_and_place.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `visual_pick_and_place` extension."""
    ...
    if args_cli.show_presets:
        from visual_pick_and_place.utils.preset_cli import enumerate_task_presets
