...
def compute_beta(iteration: int, schedule: str, num_iterations: int) -> float:
    """Return beta for the given DAgger iteration (1-indexed)."""
    if schedule == "inv_sqrt":
        return 1.0 / (iteration**0.5)
    if schedule == "linear":
        return max(0.0, 1.0 - (iteration - 1) / max(num_iterations, 1))
...
def main() -> None:
    ...
    with launch_simulation(env_cfg, args_cli):
        ...
        rollout_policy = load_rollout_policy(current_checkpoint, device)
        ...
        for iteration in range(start_iteration, num_iterations + 1):
            beta = compute_beta(iteration, beta_schedule, num_iterations)
